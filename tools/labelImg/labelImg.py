...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, defaultFilename=None, defaultPrefdefClassFile=None, defaultPrefdefPoseFile=None):
        ...
        self.loadPredefinedPoses(defaultPrefdefPoseFile)

        # Main widgets and related state.
        self.labelDialog = LabelDialog(parent=self, listItem=self.labelHist)
        self.poseDialog = LabelDialog(parent=self, listItem=self.poseHist)
        ...
        # Create a widget for using default pose
        self.useDefaultPoseCheckbox = QCheckBox(u'Use default pose')
        self.useDefaultPoseCheckbox.setChecked(False)
        self.defaultPoseTextLine = QLineEdit()
        useDefaultPoseQHBoxLayout = QHBoxLayout()
        useDefaultPoseQHBoxLayout.addWidget(self.useDefaultPoseCheckbox)
        useDefaultPoseQHBoxLayout.addWidget(self.defaultPoseTextLine)
        useDefaultPoseContainer = QWidget()
        useDefaultPoseContainer.setLayout(useDefaultPoseQHBoxLayout)
        ...
        self.truncButton.stateChanged.connect(self.t_btnstate)
        ...
        listLayout.addWidget(useDefaultPoseContainer)
    ...
    def editLabel(self):
        ...
        sl = item.text().split('+');
        text = self.labelDialog.popUp(sl[0])
        text_p = self.poseDialog.popUp(sl[1])
        if text is not None and text_p is not None:
            item.setText(text+'+'+text_p)
            self.setDirty()
    ...
    def newShape(self):
        ...
        if not self.useDefaultPoseCheckbox.isChecked() or not self.defaultPoseTextLine.text():
            if len(self.poseHist) > 0:
                self.poseDialog = LabelDialog(
                    parent=self, listItem=self.poseHist)

            # Sync single class mode from PR#106
            if self.singleClassMode.isChecked() and self.lastPose:
                text_p = self.lastPose
            else:
                text_p = self.poseDialog.popUp(text=self.prevPoseText)
                self.lastPose = text_p
        else:
            text_p = self.defaultPoseTextLine.text()

        # Add Chris
        self.diffcButton.setChecked(False)
        if text is not None and text_p is not None:
            self.prevLabelText = text
            self.prevPoseText = text_p
            generate_color = generateColorByText(text+'+'+text_p)
            shape = self.canvas.setLastLabel(text+'+'+text_p, generate_color, generate_color)
            self.addLabel(shape)
            ...
            if text_p not in self.poseHist:
                self.poseHist.append(text_p)
        else:
    ...
    def loadPredefinedPoses(self, predefPosesFile):
        if os.path.exists(predefPosesFile) is True:
            with codecs.open(predefPosesFile, 'r', 'utf8') as f:
                for line in f:
                    line = line.strip()
                    if self.poseHist is None:
                        self.poseHist = [line]
                    else:
                        self.poseHist.append(line)
